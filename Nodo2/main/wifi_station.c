...
/*i2c */
// uint8_t data_request[18] = {0x27,0x9F};
...
void monitor_state_task(void *pvParams)
{
    ...
    while(1)
    {
        ...
        // 1) abrir conexion y leer cabeceras
        ...
        if (status_code == 200 && content_length > 0) {
            // 2) leer TODO el cuerpo de una vez
            ...
            ESP_LOGW(TAG, "HTTP %d o contenido invalido", status_code);
        }
...
{
    uint32_t hum1024, hum_whole, hum_frac;
    /*usamos memoria dinamica para evitar situaciones como overflow en la tarea */
    float *hum_array = (float *)malloc(sizeof(float)* HISTORY_LEN);//[HISTORY_LEN];
    char *payload = NULL;
    // 1) Inicialización I²C y BME280
    ...
    while (1)
    {
        ...
        for (int8_t i = 0; i < HISTORY_LEN; i++) {
            hum1024 = readBME280Humidity(i2c_dev);
            hum_whole = hum1024 >> 10;
            hum_frac  = ((hum1024 & 0x3FF) * 100) >> 10;
            hum_array[i] = hum_whole + hum_frac / 100.0f;
            vTaskDelay(pdMS_TO_TICKS(SAMPLE_DELAY_MS));  // Espera entre muestras
        }
        ...
        char *p = payload;
        for (int8_t i = 0; i < HISTORY_LEN; i++) {
            int w = snprintf(p, VALUE_STR_LEN, "%.2f", hum_array[i]);
            p += w;
            if (i < HISTORY_LEN - 1)
                *p++ = TOKEN_STRING;  // Agrega separador
        }
        ...
        if (client) {
            ...
            if (esp_http_client_perform(client) == ESP_OK) {
                ...
                ESP_LOGI(TAG, "POST HUM OK, STATUS=%d, payload=%s", status, payload);
            } else {
                ESP_LOGE(TAG, "Error en POST humidity history");
            }
            esp_http_client_cleanup(client);
        } else {
        ...
        free(payload);
        //esperar 5 segundos en cada envio
